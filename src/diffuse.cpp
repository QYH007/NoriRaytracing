...
public:
    ...
    virtual Color3f evalMipmap(const BSDFQueryRecord &bRec, float mipmapLevel) const override {
        /* This is a smooth BRDF -- return zero if the measure
           is wrong, or when queried for illumination on the backside */
        if (bRec.measure != ESolidAngle
            || Frame::cosTheta(bRec.wi) <= 0
        ...
        /* The BRDF is simply the albedo / pi */
        return m_albedo->evalMipmap(bRec.uv, mipmapLevel) * INV_PI;
    }
    ...
    virtual Color3f sampleMipmap(BSDFQueryRecord &bRec, const Point2f &sample, float mipmapLevel) const override {
        ...
        return m_albedo->evalMipmap(bRec.uv, mipmapLevel);
    }
