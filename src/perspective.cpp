...
public:
    ...
    virtual void activate() override {
        ...
        dxCamera = m_sampleToCamera * (Point3f(1, 0, 0)) - m_sampleToCamera * (Point3f(0, 0, 0));
        dyCamera = m_sampleToCamera * (Point3f(0, 1, 0)) - m_sampleToCamera * (Point3f(0, 0, 0));
    ...
    Color3f sampleRay(Ray3f &ray,
        ...
        // nearP = sampled point in Camera coordinate on near clip
        // Made from sample point (0.1, 0.2)[sample coordinate] -> 
        // (0.1, 0.2)*(720*1080) = (72, 216)[raster coordinate] ->
        // raster2camara *  (72, 216, 0) = point in [camara coordinate]
        // 
        ...
        // direction = nearP - camara.Origin = [nearP - (0, 0, 0)].normalized()

        Vector3f d = nearP.normalized();
        float invZ = 1.0f / d.z();

        // compute ray in world coordinate
        // multiple with m_cameraToWorld
        ...
        Point3f nearPx = nearP+dxCamera;
        Point3f nearPy = nearP+dyCamera;
        Vector3f dx = nearPx.normalized();
        Vector3f dy = nearPy.normalized();
        ray.rxDirection = m_cameraToWorld * dx;
        ray.ryDirection = m_cameraToWorld * dy;
